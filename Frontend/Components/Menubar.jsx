...
const Menubar = () => {
  ...
  return (
    <div className=" flex flex-col justify-start md:justify-center items-center gap-2 min-w-fit min-h-fit md:hidden bg-white py-2 px-3 rounded-md relative" >
      {
        !isMenuOpen ? (<button className="w-10" onClick={()=>setIsMenuOpen(true)} ><FaBars className="text-2xl w-full text-black"/></button>):
        (<button className="w-10" onClick={()=>setIsMenuOpen(false)}  ><FaTimes className="w-full text-2xl text-black"/></button>)
      } 
